...
class InferenceCollater(object):
    ...
    def __call__(self, data_list):
        ...
        if self.tokenizer.use_adameta:
            prompts = []
            for i, p in enumerate(selfies):
                num_metaqueries = data_list[i].rdmol2selfies.shape[0]
                suffix = (
                    "\n<begin_of_mol>"
                    + "".join([f"<mol{j}>" for j in range(num_metaqueries)])
                    + "<end_of_mol>"
                )
                prompts.append(p + suffix)
        else:
            if self.tokenizer.num_metaqueries <= 0:
                ...
                suffix = (
                    "\n<begin_of_mol>"
                    + "".join([f"<mol{i}>" for i in range(self.tokenizer.num_metaqueries)])
                    + "<end_of_mol>"
                )

            prompts = [p + suffix for p in selfies]

        selfie_batch = self.tokenizer(prompts, padding='max_length', return_tensors='pt', max_length=self.max_sf_tokens, truncation=True, add_special_tokens=True)
...
class QM9UniMolVersion(Dataset):
    ...
    '''
    def __init__(self, dataset, max_atoms, is_train=True):
...
#         dataset = QM9Dataset(root=root, selfies_tokenizer=selfies_tokenizer)
#         self.dataset = dataset
...
#         self.max_sf_tokens = max(max(selfies_lens) + 2 + 5, 72) # +2 because of the bos and eos token; +5 to enlarge the space of molecule sampling
...
#         self.train_dataset = QM9UniMolVersion(dataset.index_select(train_idx), max_atoms)
#         # self.valid_dataset = QM9UniMolVersion(dataset.index_select(valid_idx), max_atoms)
#         # self.test_dataset = QM9UniMolVersion(dataset.index_select(test_idx), max_atoms)
#         self.test_dataset = QM9UniMolVersion(dataset.index_select(test_idx), max_atoms, is_train=False)
...
if __name__ == '__main__':
    ...
    if False:
        dm = QM9DM('./data/qm9v5', num_workers=2, batch_size=4, selfies_tokenizer=tokenizer)
        train_loader = dm.train_dataloader()

        ## this is a visual sanity check
        reverse_map = {v: k for k, v in dm.train_dataset.dictionary.indices.items()}
        for batch in train_loader:
            d3_mol_batch, selfies_batch = batch
            rdmol2selfies = d3_mol_batch.rdmol2selfies
            rdmol2selfies_mask = d3_mol_batch.rdmol2selfies_mask
            batch_size, atom_len = rdmol2selfies.shape[:2]
            selfies = d3_mol_batch.selfies
            for i in range(batch_size):
                l_s = list(sf.split_selfies(selfies[i]))
                for j in range(atom_len):
                    if rdmol2selfies_mask[i, j]:
                        atom_index = int(d3_mol_batch.atom_vec[i, j])
